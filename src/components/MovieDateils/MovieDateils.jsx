...
export const MovieDateils = () => {
  ...
  return (
    <Section>
      <DivContainer>
        <Button variant="contained" style={{marginBottom: 20, backgroundColor: 'orange', color: 'black'}}>
        ...
        <h3>Genres</h3>
        <ul>
          {movie.genres && movie.genres.map(({id, name}) => <li key={id}>{name}</li>)}
        </ul>
            </div>
        </MovieContainer>
        <div>
          <ListInform>
            <li>
               <Button variant="contained" style={{ backgroundColor: '#b63810', color: 'white'}}>
